class Population {
  ...
  //update all players
   update() {
    for (var i = 0; i< this.players.length; i++) {
      //this if only runs when the player reached the goal as min step is set to 10,000 and players can't reach that number of steps.
      if (this.players[i].brain.step > this.minStep) {//if the player has already taken more steps than the best player has taken to reach the goal
        this.players[i].dead = true;//then it dies this allows the program to don't waste time on the players that are just running around
      } else {
  ...
  naturalSelection() {
    ...
    //the best player lives on
    newPlayers[0] = this.players[this.bestPlayer].giveMeBaby();
    newPlayers[0].isBest = true;
    for (var i = 1; i< populationSize; i++) {
      //select parent based on fitness this is the exact same logic as the roullete function
      var parent = this.selectParent();

      //get baby from them
      newPlayers[i] = parent.giveMeBaby();
    }
  ...
  setBestPlayer() {
    ...
    if (max > this.bestFitness) {
      ...
      this.genPlayers.push(this.players[this.bestPlayer].giveMeBaby());
    }
...
//---------------------------------------------------------------------------------------------------------------------------------------------
//The moves increase only at a max of 120 moves and also if the solution wasn't and if the solution wasn't found at the time.
 increaseMoves() {
