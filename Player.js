class Player{
  constructor(){
    ...
    this.fitness = 0;
    this.nodes = [];
    this.fading = false; //This is for when they die.
    this.brain = new Brain(numberOfSteps);
    this.setNodes();
  }

  setNodes() {
    this.nodes[0] = new Node(tiles[6][7]); //The starting position of every player
    this.nodes[1] = new Node(tiles[17][2]); //The first square of the goal
  ...
  move(){
    ...
    for (var i = 0; i< solids.length; i++) {
      temp = solids[i].restrictMovement(this.pos, createVector(this.pos.x+this.size, this.pos.y+this.size), temp); //If it hits itself into a wall it looks for a new move to make.
    }
    this.pos.add(temp);

  }

//Checks if the player crashed into a dot, a wall or if he reached the goal.
checkCollisions() {
  ...
  if (this.reachedGoal) {//if the dot reached the goal then the fitness is based on the amount of steps it took to get there
    ...
    for (var i = this.nodes.length-1; i>=0; i--) {
      if (!this.nodes[i].reached) {
        estimatedDistance = this.nodes[i].distToFinish; //Our goal is to move the player all the way to the end, so we calculate the fitness based on the distance to the end
        estimatedDistance += dist(this.pos.x, this.pos.y, this.nodes[i].pos.x, this.nodes[i].pos.y); //We also take into account how much the player moved from it's initial position to where he ended to avoid moving in circles.
      }
    }
    if (this.deathByDot) {
      estimatedDistance *= 0.9; //We punished the players that died to an enemy
    }
